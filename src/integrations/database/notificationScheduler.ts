...
export class NotificationScheduler {
  ...
  static async scheduleNotification(
    ...
    try {
      ...
      console.log(`📋 Notification scheduled: ${queueItem.id} for ${scheduledFor?.toISOString() || 'immediate delivery'}`);
  ...
  static async scheduleBulkNotifications(
    ...
    try {
      ...
      console.log(`📋 ${notifications.length} notifications scheduled in bulk`);
      return ids;
    } catch (error) {
  ...
  static async processQueue() {
    ...
    try {
      ...
      console.log(`📤 Processing ${itemsToProcess.length} notifications...`);
  ...
  static async processNotificationItem(item: NotificationQueueItem) {
    try {
      ...
      if (result) {
        ...
        console.log(`✅ Notification sent successfully: ${item.id}`);
      } else {
      ...
      if (item.attempts >= item.maxAttempts) {
        ...
        console.error(`❌ Notification failed permanently: ${item.id} - ${errorMessage}`);
        ...
        console.warn(`⚠️ Notification failed, will retry in ${delayMinutes} minutes: ${item.id} - ${errorMessage}`);
      }
    }
  }
  ...
  static cleanupExpiredItems() {
    ...
    if (removedCount > 0) {
      console.log(`🧹 Cleaned up ${removedCount} old notification items`);
    }
  }
  ...
  static async scheduleRecurringNotification(
    ...
    try {
      ...
      console.log(`🔄 Scheduled ${scheduledIds.length} recurring notifications`);
      return scheduledIds;
    } catch (error) {
